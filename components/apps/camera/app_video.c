...
{
    uint32_t buf_size = app_camera_video.camera_buf_hes * app_camera_video.camera_buf_ves * (APP_VIDEO_FMT_RGB565 == APP_VIDEO_FMT_RGB565 ? 2 : 3);
...
esp_err_t app_video_stream_task_start(int video_fd, int core_id)
{
    ...
    BaseType_t result = xTaskCreatePinnedToCore(video_stream_task, "video stream task", VIDEO_TASK_STACK_SIZE, (void *)video_fd, VIDEO_TASK_PRIORITY, &app_camera_video.video_stream_task_handle, core_id);
